...
test('Offline Authority Runtime enforces scoped policy, immutable evidence, and durable replay', async () => {
  await withOfflineSyncFixture(async fixture => {
    const base=`/api/v1/retail/locations/${fixture.location.id}/offline-authorities`, encode=(pair:ReturnType<typeof generateKeyPairSync>)=>pair.publicKey.export({format:'der',type:'spki'}).toString('base64'), payload={commandId:'authority-http-issue',terminalId:fixture.terminal.id,userId:'operator-1',expiresAt:new Date(Date.now()+60_000).toISOString(),permitCount:2,productIds:[fixture.product.id]}, count=()=>({authorities:(fixture.database.prepare('SELECT COUNT(*) AS count FROM retail_offline_authorities').get()as{count:number}).count,prices:(fixture.database.prepare('SELECT COUNT(*) AS count FROM retail_offline_authority_product_prices').get()as{count:number}).count,permits:(fixture.database.prepare('SELECT COUNT(*) AS count FROM retail_offline_authority_permits').get()as{count:number}).count,revocations:(fixture.database.prepare('SELECT COUNT(*) AS count FROM retail_offline_authority_revocations').get()as{count:number}).count,receipts:(fixture.database.prepare('SELECT COUNT(*) AS count FROM retail_offline_operational_command_receipts').get()as{count:number}).count,audits:(fixture.database.prepare("SELECT COUNT(*) AS count FROM audit_events WHERE action LIKE 'retail.offline_authority_%'").get()as{count:number}).count})
    equal((await fixture.app.inject({method:'GET',url:base})).statusCode,401); equal((await fixture.app.inject({method:'POST',url:base,payload})).statusCode,401); equal((await request(fixture.app,fixture.sessions['operator-1']!,{method:'POST',url:base,payload})).statusCode,403); equal((await fixture.app.inject({method:'POST',url:base,payload,headers:{cookie:`madina-session=${fixture.session}`,origin:'https://untrusted.example'}})).statusCode,403)
    const before=count(); for(const invalid of [{...payload,commandId:'   '},{...payload,terminalId:' '},{...payload,userId:' '},{...payload,expiresAt:'not-a-date'},{...payload,permitCount:0},{...payload,productIds:[]},{...payload,productIds:[' ']},{...payload,productIds:[fixture.product.id,fixture.product.id]},{...payload,locationId:fixture.otherLocation.id,authorityVersion:9,actorUserId:'operator-1'}])equal((await request(fixture.app,fixture.session,{method:'POST',url:base,payload:invalid})).statusCode,400); deepEqual(count(),before)
    equal((await request(fixture.app,fixture.session,{method:'POST',url:base,payload:{...payload,commandId:'invalid-user',userId:'missing-user'}})).statusCode,409); deepEqual(count(),before)
    const foreignPair=generateKeyPairSync('ed25519'); await fixture.catalog.setPrice(fixture.product.id,fixture.otherLocation.id,100,fixture.context); const foreignTerminal=await fixture.offline.enrollTerminal({commandId:'foreign-authority-terminal',locationId:fixture.otherLocation.id,keyAlgorithm:RETAIL_OFFLINE_SIGNATURE_ALGORITHM,publicKey:encode(foreignPair)},fixture.context), foreignAuthority=await fixture.offline.issueAuthority({commandId:'foreign-authority',terminalId:foreignTerminal.id,userId:'operator-1',locationId:fixture.otherLocation.id,expiresAt:new Date(Date.now()+60_000),permitCount:1,productIds:[fixture.product.id]},fixture.context)
    const afterForeign=count(); equal((await request(fixture.app,fixture.session,{method:'POST',url:base,payload:{...payload,commandId:'foreign-terminal',terminalId:foreignTerminal.id}})).statusCode,404); equal((await request(fixture.app,fixture.session,{method:'GET',url:`${base}/${foreignAuthority.id}`})).statusCode,404); equal((await request(fixture.app,fixture.session,{method:'GET',url:`${base}/${foreignAuthority.id}/permits`})).statusCode,404); equal((await request(fixture.app,fixture.session,{method:'POST',url:`${base}/${foreignAuthority.id}/revoke`,payload:{commandId:'foreign-revoke',reason:'no'}})).statusCode,404); deepEqual(count(),afterForeign)
    const created=await request(fixture.app,fixture.session,{method:'POST',url:base,payload}), issued=(created.json()as{authority:{id:string;terminalKeyVersion:number;currencyCode:string;currencyExponent:number;permitCount:number}}).authority; equal(created.statusCode,201); deepEqual({key:issued.terminalKeyVersion,currency:issued.currencyCode,exponent:issued.currencyExponent,permits:issued.permitCount},{key:1,currency:'USD',exponent:2,permits:2}); const afterIssue=count(); deepEqual((await request(fixture.app,fixture.session,{method:'POST',url:base,payload})).json(),created.json()); deepEqual(count(),afterIssue); await fixture.access.grant('admin-1',fixture.location.id,fixture.context); equal((await request(fixture.app,fixture.sessions['admin-1']!,{method:'POST',url:base,payload})).statusCode,409); deepEqual(count(),afterIssue); equal((await request(fixture.app,fixture.session,{method:'POST',url:base,payload:{...payload,permitCount:3}})).statusCode,409); deepEqual(count(),afterIssue)
    const list=(await request(fixture.app,fixture.session,{method:'GET',url:base})).json()as{authorities:Array<{authorityId:string}>}; equal(list.authorities.some(authority=>authority.authorityId===issued.id),true); const detailResponse=await request(fixture.app,fixture.session,{method:'GET',url:`${base}/${issued.id}`}), detail=detailResponse.json()as{authority:{productPrices:Array<{productId:string;unitPriceMinor:number}>;permitCounts:{available:number;conflictPending:number;accepted:number}}}; equal(detailResponse.statusCode,200); equal(detailResponse.body.includes('publicKey'),false); equal(detailResponse.body.includes('payload_hash'),false); deepEqual(detail.authority.productPrices,[{productId:fixture.product.id,unitPriceMinor:100}])
    const permits=await fixture.offline.listPermits(issued.id), accepted=offlineEnvelope(fixture,permits[0]!,{authorityId:issued.id,offlineOperationId:'authority-http-accepted',proposedSaleId:'authority-http-accepted-sale'}), conflict=offlineEnvelope(fixture,permits[1]!,{authorityId:issued.id,offlineOperationId:'authority-http-conflict',proposedSaleId:'authority-http-conflict-sale',lines:[{id:'authority-http-conflict-item',productId:fixture.product.id,quantity:99,unitPriceMinor:100}],cashAllocation:{id:'authority-http-conflict-payment',method:'cash',amountMinor:9900,ordinal:0},subtotalMinor:9900,payableTotalMinor:9900}); equal((await postOffline(fixture,signedOfflinePayload(accepted,fixture.pair.privateKey))).statusCode,201); equal((await postOffline(fixture,signedOfflinePayload(conflict,fixture.pair.privateKey))).statusCode,409)
    const permitRead=(await request(fixture.app,fixture.session,{method:'GET',url:`${base}/${issued.id}/permits`})).json()as{permits:Array<{sequence:number;status:string}>}; deepEqual(permitRead.permits.map(({sequence,status})=>({sequence,status})),[{sequence:0,status:'CONSUMED_ACCEPTED'},{sequence:1,status:'CONSUMED_CONFLICT_PENDING'}]); const afterEvidence=(await request(fixture.app,fixture.session,{method:'GET',url:`${base}/${issued.id}`})).json()as{authority:{permitCounts:{available:number;conflictPending:number;accepted:number}}}; deepEqual(afterEvidence.authority.permitCounts,{available:0,conflictPending:1,accepted:1})
    const revokeUrl=`${base}/${issued.id}/revoke`, revocation={commandId:'authority-http-revoke',reason:'trust lost'}, revoked=await request(fixture.app,fixture.session,{method:'POST',url:revokeUrl,payload:revocation}); equal(revoked.statusCode,200); const afterRevoke=count(); deepEqual((await request(fixture.app,fixture.session,{method:'POST',url:revokeUrl,payload:revocation})).json(),revoked.json()); deepEqual(count(),afterRevoke); equal((await request(fixture.app,fixture.session,{method:'POST',url:revokeUrl,payload:{...revocation,reason:'changed'}})).statusCode,409); await fixture.offline.revokeTerminal(fixture.terminal.id,'later terminal revocation','authority-history-terminal-revoke',fixture.context); const afterTrustChange=count()
    deepEqual((await request(fixture.app,fixture.session,{method:'POST',url:base,payload})).json(),created.json()); deepEqual(count(),afterTrustChange); equal((await request(fixture.app,fixture.session,{method:'POST',url:base,payload:{...payload,commandId:'revoked-terminal-new'}})).statusCode,409); const revokedDetail=(await request(fixture.app,fixture.session,{method:'GET',url:`${base}/${issued.id}`})).json()as{authority:{revoked:boolean;revocation:{reason:string}}}; equal(revokedDetail.authority.revoked,true); equal(revokedDetail.authority.revocation.reason,'trust lost'); await fixture.access.revoke('manager-1',fixture.location.id,fixture.context); equal((await request(fixture.app,fixture.session,{method:'POST',url:base,payload})).statusCode,403)
  })
})
...
test('Offline Terminal Runtime enforces scoped access, hides keys, and preserves durable operational replay', async () => {
  await withOfflineSyncFixture(async fixture => {
    ...
    equal(created.statusCode,201); equal(typeof terminal.id,'string'); equal(terminal.locationId,fixture.location.id); const afterEnroll=count(); deepEqual(await request(fixture.app,fixture.session,{method:'POST',url:base,payload}).then(response=>response.json()),created.json()); deepEqual(count(),afterEnroll)
    ...
    const detailResponse=await request(fixture.app,fixture.session,{method:'GET',url:`${base}/${terminal.id}`}), detailText=detailResponse.body, detail=detailResponse.json()as{terminal:{terminalId:string;keys:Array<{keyVersion:number;current:boolean}>}}
    equal(detailResponse.statusCode,200); equal(detail.terminal.terminalId,terminal.id); equal(Object.hasOwn(detail.terminal,'id'),false); equal(detailText.includes(payload.publicKey),false); equal(detailText.includes('privateKey'),false); deepEqual(detail.terminal.keys.map(key=>({version:key.keyVersion,current:key.current})),[{version:1,current:true}])
