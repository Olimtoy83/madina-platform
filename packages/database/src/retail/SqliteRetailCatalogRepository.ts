...
export class SqliteRetailCatalogRepository {
  ...
  async setPrice(productId:string,locationId:string,unitPriceMinor:number,context:CommandContext):Promise<{productId:string;locationId:string;unitPriceMinor:number}>{if(!Number.isSafeInteger(unitPriceMinor)||unitPriceMinor<=0)throw new Error('Retail Product price must be a positive safe integer.');return this.transaction(async()=>{const product=await this.findProduct(productId);if(!product)throw new Error('Retail Product not found.');if(product.status!=='active')throw new Error('Retail Product is inactive.');const location=this.database.prepare('SELECT status FROM retail_locations WHERE id=?').get(locationId)as {status:string}|undefined;if(!location)throw new Error('Retail Location not found.');if(location.status!=='active')throw new Error('Retail Location is inactive.');const now=new Date().toISOString();this.database.prepare('INSERT INTO retail_product_prices(product_id,location_id,unit_price_minor,created_at,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(product_id,location_id) DO UPDATE SET unit_price_minor=excluded.unit_price_minor,updated_at=excluded.updated_at').run(productId,locationId,unitPriceMinor,now,now);this.audit(context,productId,'retail.product_price_set',{locationId,unitPriceMinor});return{productId,locationId,unitPriceMinor}})}
  async findPrice(productId:string,locationId:string):Promise<number|undefined>{const row=this.database.prepare('SELECT unit_price_minor FROM retail_product_prices WHERE product_id=? AND location_id=?').get(productId,locationId)as {unit_price_minor:number}|undefined;return row?.unit_price_minor}
