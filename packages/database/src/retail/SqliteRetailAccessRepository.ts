...
interface Row { id: string; code: string; name: string; type: RetailLocationType; status: RetailLocationStatus; currency_code:string|null; currency_exponent:number|null; created_at: string; updated_at: string }
const toLocation = (row: Row): RetailLocation => ({ id: row.id, code: row.code, name: row.name, type: row.type, status: row.status, currencyCode:row.currency_code??undefined,currencyExponent:row.currency_exponent??undefined, createdAt: new Date(row.created_at), updatedAt: new Date(row.updated_at) })
...
export class SqliteRetailAccessRepository {
  ...
  async listLocations(): Promise<RetailLocation[]> { return (this.database.prepare('SELECT id, code, name, type, status, currency_code, currency_exponent, created_at, updated_at FROM retail_locations ORDER BY code').all() as unknown as Row[]).map(toLocation) }
  async findLocation(id: string): Promise<RetailLocation | undefined> { const row = this.database.prepare('SELECT id, code, name, type, status, currency_code, currency_exponent, created_at, updated_at FROM retail_locations WHERE id = ?').get(id) as Row | undefined; return row ? toLocation(row) : undefined }
  ...
  async listPermittedLocations(userId: string): Promise<RetailLocation[]> { return (this.database.prepare(`SELECT l.id, l.code, l.name, l.type, l.status, l.currency_code, l.currency_exponent, l.created_at, l.updated_at FROM retail_locations l JOIN retail_user_location_grants g ON g.location_id = l.id WHERE g.user_id = ? AND g.revoked_at IS NULL AND l.status = 'active' ORDER BY l.code`).all(userId) as unknown as Row[]).map(toLocation) }
  ...
  async configureCurrency(id:string,currencyCode:string,currencyExponent:number,context:CommandContext):Promise<RetailLocation>{if(!/^[A-Z]{3}$/.test(currencyCode)||!Number.isSafeInteger(currencyExponent)||currencyExponent<0||currencyExponent>9)throw new Error('Retail Location currency configuration is invalid.');return this.transaction(async()=>{const existing=await this.findLocation(id);if(!existing)throw new Error('Retail Location not found.');if(existing.status!=='active')throw new Error('Retail Location is inactive.');const now=new Date();this.database.prepare('UPDATE retail_locations SET currency_code=?,currency_exponent=?,updated_at=? WHERE id=?').run(currencyCode,currencyExponent,now.toISOString(),id);this.audit(context,id,'retail.location_updated',{currencyCode,currencyExponent});return {...existing,currencyCode,currencyExponent,updatedAt:now}})}
